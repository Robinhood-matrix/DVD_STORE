...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="container" style="margin-left: 5.5%; margin-top: -3%">
        ...
        <asp:SqlDataSource ID="SqlDataSource4" runat="server" ConnectionString="<%$ ConnectionStrings:dvd_storeConnectionString %>" DeleteCommand="DELETE FROM [Customer] WHERE [CustomerId] = @CustomerId" InsertCommand="INSERT INTO [Customer] ([CustomerId], [CustomerName], [Address], [ContactNo], [Email], [CustomerType]) VALUES (@CustomerId, @CustomerName, @Address, @ContactNo, @Email, @CustomerType)" SelectCommand="SELECT Member.MemberLastName, Member.MemberFirstName, Loan.DateReturned, Loan.DateDue, Loan.DateOut, DVDTitle.DVDTitle FROM Member INNER JOIN Loan ON Member.MemberNumber = Loan.MemberNumber INNER JOIN DVDCopy ON Loan.CopyNumber = DVDCopy.CopyNumber INNER JOIN DVDTitle ON DVDCopy.DVDNumber = DVDTitle.DVDNumber" UpdateCommand="UPDATE [Customer] SET [CustomerName] = @CustomerName, [Address] = @Address, [ContactNo] = @ContactNo, [Email] = @Email, [CustomerType] = @CustomerType WHERE [CustomerId] = @CustomerId">
            <DeleteParameters>
    ...
    <div class="container" style="margin-left: 5.5%; margin-top: -3%">
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:dvd_storeConnectionString %>" DeleteCommand="DELETE FROM [Customer] WHERE [CustomerId] = @CustomerId" InsertCommand="INSERT INTO [Customer] ([CustomerId], [CustomerName], [Address], [ContactNo], [Email], [CustomerType]) VALUES (@CustomerId, @CustomerName, @Address, @ContactNo, @Email, @CustomerType)" SelectCommand="SELECT DVDTitle.DVDTitle, DVDCopy.CopyNumber, Member.MemberNumber, Loan.LoanTypeNumber, Loan.DateOut FROM Loan INNER JOIN Member ON Loan.MemberNumber = Member.MemberNumber INNER JOIN DVDCopy ON Loan.CopyNumber = DVDCopy.CopyNumber INNER JOIN DVDTitle ON DVDCopy.DVDNumber = DVDTitle.DVDNumber WHERE (Loan.DateOut &gt;= DATEADD(day, - 31, GETDATE())) AND (Loan.DateOut &lt;= GETDATE()) AND (Loan.MemberNumber = @MemberNumber)" UpdateCommand="UPDATE [Customer] SET [CustomerName] = @CustomerName, [Address] = @Address, [ContactNo] = @ContactNo, [Email] = @Email, [CustomerType] = @CustomerType WHERE [CustomerId] = @CustomerId">
            ...
            <SelectParameters>
                <asp:ControlParameter ControlID="mLastTextBox" Name="MemberNumber" PropertyName="Text" />
            </SelectParameters>
            <UpdateParameters>
                <asp:Parameter Name="CustomerName" Type="String" />
                <asp:Parameter Name="Address" Type="String" />
   ...
   <h3 style="margin-left: 6.5%; margin-top: 2%">&nbsp; Issue DVD Copy</h3>
        <div class="row" style="margin-left: 7%">
        <p>
            <asp:GridView ID="GridView1" runat="server" AllowSorting="True" AutoGenerateColumns="False" CellPadding="4" DataKeyNames="LoanNumber" DataSourceID="sqlDataSource2" ForeColor="#333333" GridLines="None" Height="218px" Width="1341px">
                <AlternatingRowStyle BackColor="White" ForeColor="#284775" />
                <Columns>
                    <asp:BoundField DataField="LoanNumber" HeaderText="LoanNumber" ReadOnly="True" SortExpression="LoanNumber" />
                    <asp:BoundField DataField="DateOut" HeaderText="DateOut" SortExpression="DateOut" />
            ...
            <asp:SqlDataSource ID="sqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:dvd_storeConnectionString %>" SelectCommand="SELECT Loan.LoanNumber, Loan.DateOut, Member.MemberFirstName, Member.MemberAddress FROM Loan INNER JOIN Member ON Loan.MemberNumber = Member.MemberNumber WHERE (NOT (Loan.DateOut &gt;= DATEADD(day, - 31, GETDATE()))) AND (Loan.DateOut &lt;= GETDATE())"></asp:SqlDataSource>
         </p>
    </div>
    <p>
        &nbsp;</p>
