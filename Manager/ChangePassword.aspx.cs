...
namespace MasterPageDemo.Admin
{
    public partial class ChangePassword : System.Web.UI.Page
    {
        ...
        protected void changePasswordBtn_Click(object sender, EventArgs e)
        {
            ...
            string constr = ConfigurationManager.ConnectionStrings["dvd_storeConnectionString"].ConnectionString;
            ...
            try
            {
                ...
                {
                    con.Open();
                    string qry = "select * from Users where UserName='" + Uname + "' and UserPassword='" + oldpass + "'";
                    ...
                    if (sdr.Read())
                    {
                        if (newpass == conpass)
                        {
                            ...
                            string connection = ConfigurationManager.ConnectionStrings["dvd_storeConnectionString"].ConnectionString;

                            using (var conn = new SqlConnection(connection))
                            {
                                conn.Open();
                                string updSql = @"UPDATE User SET Userpassword = '" + newpass + "'  WHERE UserName = '" + Uname + "'";
